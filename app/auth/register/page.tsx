    ...
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4 overflow-hidden relative">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div animate={{ scale: [1,1.2,1], opacity: [0.15,0.25,0.15] }} transition={{ duration: 7, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute -top-32 -right-32 w-96 h-96 bg-indigo-600 rounded-full blur-3xl" />
        <motion.div animate={{ scale: [1,1.15,1], opacity: [0.1,0.2,0.1] }} transition={{ duration: 9, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            {[
              ...
              { label: 'E-posta', type: 'email', value: email, set: setEmail, placeholder: 'ornek@email.com', delay: 0.35 },
            ...
            {error && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                className={`border rounded-xl px-4 py-2.5 text-sm ${error.includes('başarılı') ? 'bg-green-900/30 border-green-700/50 text-green-400' : 'bg-red-900/30 border-red-700/50 text-red-400'}`}>
                {error}
              </motion.div>
            )}
          ...
          <p className="text-center text-sm text-gray-500 mt-4">
            ...
            <Link href="/auth/login" className="text-blue-400 hover:text-blue-300 font-medium">Giriş Yap</Link>
          </p>
        </motion.div>
      </motion.div>
    </div>
